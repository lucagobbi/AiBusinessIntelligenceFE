<script lang="ts">
  import {Button, Heading, Helper, Input, Label, Spinner, DarkMode, Footer, FooterCopyright} from "flowbite-svelte";
...
<main class="dark:bg-slate-800 h-screen">
  ...
  <div class="flex flex-col justify-center items-center">
    <Label class="space-y-2 mt-3 w-9/12">
      <Input type="text" placeholder="Type here your query" size="lg" />
      <Helper class="pt-2 text-center">Write your query as if you are asking another human, not a machine.</Helper>
    </Label>
    <Button gradient color="blue" class="mt-4 outline-none border-none focus:outline-none w-1/5" on:click={send}>
      {#if loading}
  ...
  <Footer class="dark:bg-transparent fixed bottom-0 flex dark:justify-center w-full">
    <div class="flex">
      <p class="dark:text-stone-300 dark:text-opacity-85 text-xs mt-0.5 mr-1">Luca Gobbi - {new Date().getFullYear()} - Built with</p> <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 98.1 118" style="enable-background:new 0 0 98.1 118;" xml:space="preserve">
<style type="text/css">
